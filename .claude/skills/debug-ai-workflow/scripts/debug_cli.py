...
def execute_test_case(workflow_id: str, step_id: str, test_case: str, project_root: str) -> dict:
    ...
    print(f"[{test_case}] Spawning Sub-Agent via Claude CLI...")
    
    # Inject workDir requirement
    workspace_path_str = str(debug_run_dir.absolute())
    
    # In case the user's prompt needs the raw path (for {{workDir}} replacement), 
    # we inject it at the top, and also replace any literal {workDir} tokens if they exist.
...
def main():
    ...
    if "error" in res:
        ...
        print(f"Result: {'PASS' if res['passed'] else 'FAIL'} (Execution: {res['execution_time']:.2f}s, Validation: {res['validation_time']:.2f}s)")
        print(f"Debug Workspace: {res['workspace']}")
